...
2. Per-Segment Generation (repeated per segment)
   ...
   b. Vision Analysis + Runway Animation
      - Agent passes Flux image to Claude vision analysis
      - Vision analysis returns structured animation brief:
        ANIMATE (existing elements), ADD (atmospheric layers),
        STATIC (locked elements), LOOP TYPE, INTENSITY
      - Agent translates brief into Runway Gen-4 text prompt
      - Runway generates 10-second animated clip per segment
      - FFmpeg loops clip to fill 30-minute segment duration
      - No Ken Burns — Runway animation IS the motion
      - Output: animated clip per segment (looped to 30 min)
...
| `@asset-producer` | producer | Calls Flux (body images), ElevenLabs (VO), Sonauto (music), Runway (animation via vision analysis) |
...
| Runway Gen-4 | Photo-to-video animation (Track B) | Per second of output |
...
- [ ] Runway Gen-4 API loop output specs confirmation
...
*Last updated: session 3 — Kling replaced with Runway throughout, vision analysis + animation brief system added to Track B, image-framework.md complete (15 categories + universe modifier library), runway-framework.md complete.*
